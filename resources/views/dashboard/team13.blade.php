	...
	<div class="toolbar" id="kt_toolbar">
		<div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
			<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
				<h1 class="d-flex align-items-center text-dark fw-bolder fs-3 my-1">Dashboard Team13
							...
							<div class="card-p mt-n20 position-relative">
								<div class="row g-0">
									<div class="col bg-light-warning px-6 py-8 rounded-2 me-7 mb-7" style="display: flex; align-items: center; justify-content: space-between;">
										...
										<h2 style="margin-right: 10px;">{{ $totalwritertl }}</h2>
									</div>
									<div class="col bg-light-primary px-6 py-8 rounded-2 mb-7" style="display: flex; align-items: center; justify-content: space-between;">
										<div>
											...
											<a href="#" class="text-primary fw-bold fs-6">Not Assigned Order</a>
										</div>
										<h2 style="margin-right: 10px;">{{ $notAssignOrderCount }}</h2>
									</div>
								</div>
						...
						<div class="card-header border-0">
							...
							<div class="card-toolbar">
								...
								<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg-light-primary fw-bold w-200px" data-kt-menu="true">
                                    ...
                                    <div class="card card-xl-stretch mb-xl-8">
                                        <div class="card-header border-0">
                                            <h3 class="card-title fw-bolder text-dark" style="margin-top: 20px;">Writer</h3>
                                            <div class="card-toolbar">
...
<script>
    $(document).ready(function () {
        // Event handler for writer selection
        $('#roleSelector').change(function () {
            var writerId = $(this).val();

            // Check if a writer is selected
            if (writerId) {
                // Make an AJAX request to fetch subwriters
                fetchSubwriters(writerId);
            } else {
                // Clear team members container if no writer is selected
                updateTeamMembers([]);
            }
        });
    });

    function fetchSubwriters(writerId) {
        // Make an AJAX request to fetch subwriters based on the selected writer's ID
        $.ajax({
            url: '/fetch-subwriters',  // The URL of the route that handles the AJAX request
            type: 'GET',
            data: { tlId: writerId },
            ...
            error: function (error) {
                console.error('Error fetching subwriters:', error);
            }
        });
    }
    ...
    function updateTeamMembers(response) {
        ...
        // Check if subwriters exist in the response
        if (response && response.length > 0) {
            // Append new team members
            response.forEach(function (subwriter) {
                var memberHtml = '<p>' + subwriter.name + '</p>';

                // Append the HTML to the container
                $('#teamMembersContainer').append(memberHtml);
            });
        } else {
            // If no subwriters found, display a message
            $('#teamMembersContainer').html('<p>No subwriters found.</p>');
        }
    }
</script>
...
<script type="text/javascript">
    ...
    function drawChart() {
        // PHP variables are set here manually or fetched from a server using AJAX.
        var completeOrderPercentage = <?php echo round(($completeOrder13 / $totalwritertl) * 100); ?>;
        var notAssignPercentage = <?php echo round(($notAssignOrderCount / $totalwritertl) * 100); ?>;
        var inProgressPercentage = <?php echo round(($inprogressOrder13 / $totalwritertl) * 100); ?>;

        var data = google.visualization.arrayToDataTable([
            ['Task', 'Hours per Day'],
            ['Completed',    completeOrderPercentage],
            ['Not Assign',   notAssignPercentage],
